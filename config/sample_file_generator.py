...
def generate_sample_file(assembly_type, path, output):
    ...
    for file_name in listdir(path):
        if(assembly_type == "none"):
            ...
            df_samples.loc[index, "sample_name"] = file_name.replace(".fastq.gz", "").replace("_R1", "").replace("_R2", "")
            file_name_r1 = file_name.replace("_R1", "").replace("_R2", "").replace(".fastq.gz", "_R1.fastq.gz")
            df_samples.loc[index, "shortR1"] = path + "/" + file_name_r1
            file_name_r2 = file_name.replace("_R1", "").replace("_R2", "").replace(".fastq.gz", "_R2.fastq.gz")
            df_samples.loc[index, "shortR2"] = path + "/" + file_name_r2
        if(assembly_type == "long" or assembly_type == "hybrid"):
            df_samples.loc[index, "sample_name"] = file_name.replace("_R1", "").replace("_R2", "").replace(".fastq.gz", "")
            df_samples.loc[index, "long"] = path + "/" + file_name.replace("_R1", "").replace("_R2", "")


        index = index + 1
    df_samples.drop_duplicates(ignore_index=True, inplace=True)
